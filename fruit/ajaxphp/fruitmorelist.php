<?php
	...
	$resulttotal = $db->query("select count(fruitid) as num from t_fruit where pfruitid = '".$_GET[fruitid]."' and rstatus = 0 and status = 0");
    ...
    while ($row_fruit = mysql_fetch_array($result_fruit)) {
		...
		echo '<li>
				...
				<span onclick="addtocart('.$row_fruit[fruitid].', 1, 0);"><font>加入购物车</font></span>
			  </li>'."\n";
    }
    echo '</ul>'."\n";
    echo '</div>'."\n";
